...
const Transfer = ({selectedToken, setAction, thirdWebTokens, walletAddress}) => {
    ...
    useEffect(() => {
      ...
      const mintTo = async (walletAddress, amount) => {  
        try { 
            setAction("Buying")
          await activeThirdWebToken.mintTo(walletAddress, amount.toString().concat("000000000000000000"));
          setAction("Bought")
        } catch (error) {
    ...
    return (
        <Wrapper>
            ...
            <Row>
                <Continue onClick={() => mintTo(walletAddress, amount)}>Continue</Continue>
            </Row>
            <Row>
